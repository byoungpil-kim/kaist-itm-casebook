...
4. **PDF 다운로드는 검수 목적으로만 원문(fulltext) 페이지에 임시 노출한다.**
   - **요약(cases) 페이지에는 PDF 버튼을 두지 않는다**(build.py 다운로드바에 원문 전체 보기 버튼만).
   - **원문(fulltext) 페이지 하단 다운로드바에는 "[검수용] PDF 내려받기 ⭳" 버튼**과 바로 아래 작은 안내
     문구("PDF 변환 및 요약이 제대로 되었는지 확인을 위해 PDF 파일을 제공합니다. 최종 홈페이지에서는
     PDF 원문을 제공하지 않을 예정입니다.")를 유지한다. 이 dlbar는 각 `fulltext/{id}/index.html`에
     직접 들어있다(build.py 생성 아님) — 문구 변경 시 36개 파일 일괄 치환.
   - `pdf/{사례ID}.pdf`(학기 모음집은 해당 사례만 분할한 파일)는 유지. 정식 공개가 결정되면 사용자
     지시에 따라 버튼·안내문구·pdf/ 폴더를 제거한다.
